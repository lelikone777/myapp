...
const ContactContent = () => {
  ...
  const validate = () => {
    ...
    else if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(form.email.trim())) newErrors.email = "Некорректный email";
    if (!form.message.trim()) newErrors.message = "Добавьте сообщение";
    else if (form.message.trim().length < 20) newErrors.message = "Минимум 20 символов";
    if (form.phone.trim() && form.phone.trim().replace(/\D/g, "").length < 6) newErrors.phone = "Телефон слишком короткий";
    if (form.telegram.trim() && !/^@?[\w\d_]{3,}$/.test(form.telegram.trim()))
      newErrors.telegram = "Некорректный Telegram";
    setErrors(newErrors);
  ...
  return (
    <Section>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
              {contactLinks.map(({ href, label, icon }) => (
                <a
                  ...
                  aria-label={label}
                >
                  <span className="inline-flex h-12 w-12 items-center justify-center rounded-full border border-[var(--border)] bg-[var(--card)] transition group-hover:border-[var(--accent)] group-hover:bg-[var(--nav-hover)]">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" fill="currentColor" className="h-6 w-6" aria-hidden="true">
                      {icon}
                    </svg>
                  </span>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 px-4 py-8 sm:py-10">
          <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-[var(--shadow)]">
            <div className="max-h-[90vh] overflow-y-auto p-4 sm:p-6">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-2xl font-semibold">{t("contact.requestCta")}</h2>
                  <p className="text-sm text-[var(--muted)]">Заполните форму и я свяжусь с вами.</p>
                </div>
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className="cursor-pointer rounded-lg border border-[var(--border)] px-3 py-1 text-sm text-[var(--muted)] hover:border-[var(--accent)] hover:text-[var(--accent)]"
                >
                  Закрыть
                </button>
              </div>
              <form className="mt-4 space-y-3" onSubmit={handleSubmit}>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className={`w-full rounded-lg border bg-[var(--card)] px-3 py-2 text-[var(--foreground)] ${
                  ...
                  <div className="space-y-1">
                    <label className="text-sm text-[var(--muted)]" htmlFor="company">
                      Организация
                    </label>
                    <input
                      id="company"
                ...
                <div className="space-y-1">
                  <label className="text-sm text-[var(--muted)]" htmlFor="telegram">
                    Telegram
                  </label>
                  <input
                    id="telegram"
                    ...
                    className={`w-full rounded-lg border bg-[var(--card)] px-3 py-2 text-[var(--foreground)] ${
                      errors.telegram ? "border-red-500" : "border-[var(--border)]"
                    }`}
                  />
                  {errors.telegram && <p className="text-sm text-red-500">{errors.telegram}</p>}
                </div>
                <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-2 pt-2">
                  <button
                    ...
                    className="cursor-pointer rounded-lg bg-[var(--accent)] px-4 py-2 text-[var(--accent-foreground)] transition hover:opacity-90 disabled:opacity-60"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setOpen(false)}
                    className="cursor-pointer rounded-lg border border-[var(--border)] bg-[var(--card)] px-4 py-2 text-[var(--foreground)] transition hover:border-[var(--accent)]"
                  >
                    Отмена
                  </button>
                  {status === "error" && <p className="text-sm text-red-500">Ошибка при отправке</p>}
